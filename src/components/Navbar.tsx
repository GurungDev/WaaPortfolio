  ...
  return (
    ...
    >
      <div className="app-layout flex items-center justify-between py-3 px-4">
        ...
        >
{[
            { href: "/", label: "Home", color: "red-400" },
            { href: "/services", label: "Services", color: "green-400" },
            { href: "/ourTeam", label: "Our Team", color: "blue-400" },
            { href: "/contact_us", label: "Contact Us", color: "yellow-400" },
          ].map((link) => (
            <Link href={link.href} key={link.href} className="caption font-[200] tracking-loose leading-[1.2rem]">
              <p>{link.label}</p>
              <div className={`linkProgressBar h-2 transition-all duration-500 ease-in ${activeLink === link.href ? `w-full bg-${link.color}` : "w-0"}`}>
                <div className="" />
              </div>
            </Link>
          ))}
